...
impl TextSigner for Ed25519Signer {
    fn sign(&self, reader: &mut dyn Read) -> Result<Vec<u8>> {
        let mut buf = Vec::new();
        reader.read_to_end(&mut buf)?;
        let signature: ed25519_dalek::Signature = self.key.sign(&buf);
...
pub fn process_text_sign(
    ...
    let singer: Box<dyn TextSigner> = match format {
...
pub fn process_key_generate(format: TextSignFormat) -> Result<HashMap<&'static str, Vec<u8>>> {
    match format {
